...
class LightGCN(GeneralRecommender):
    ...
    def get_norm_adj_mat(self):
        ...
        """
        # Build adjacency matrix with COO/CSR instead of dok_matrix._update/update.
        # Newer SciPy versions removed/blocked direct DOK bulk update APIs.
        ...
        # Collapse possible duplicate entries to binary edges.
        A.data = np.ones_like(A.data, dtype=np.float32)

        # norm adj matrix
        sumArr = (A > 0).sum(axis=1)
        # add epsilon to avoid Divide by zero Warning
        diag = np.array(sumArr).flatten() + 1e-7
