...
class FREEDOM(GeneralRecommender):
    def __init__(self, config, dataset):
        ...
        if os.path.exists(mm_adj_file):
            # 历史缓存通常保存在 CPU；显式加载到 CPU 后再统一搬到目标设备，
            # 避免出现 self.mm_adj 在 CPU、而前向中的 h 在 CUDA 的设备不一致问题。
            ...
            if self.v_feat is not None and self.t_feat is not None:
                ...
                del text_adj
                del image_adj
            # 保存 CPU 版本，便于后续在不同设备上复用。
            torch.save(self.mm_adj.cpu(), mm_adj_file)

        # 普通 tensor 属性不会随着 model.to(device) 自动迁移，
        # 因此这里显式将多模态 item-item 图搬到目标设备。
    ...
    def get_norm_adj_mat(self):
        ...
        # 直接构建双向 user-item 图，不再使用 dok_matrix._update()
        ...
        L = d_mat.dot(A).dot(d_mat).tocsr()

        row = L.row if hasattr(L, "row") else L.tocoo().row
        col = L.col if hasattr(L, "col") else L.tocoo().col
        data = L.data if hasattr(L, "data") else L.tocoo().data
